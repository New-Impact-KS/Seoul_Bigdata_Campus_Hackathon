...
'''
    [데이터 행 삭제]
    인천광역시 138개 행 제거
    6251개 행 데이터는 지번주소가 시군구_행정동으로 되어있지 않은 것으로 파악
    
    [자치구명 및 행정동명이 NA 아닌 DF (cctv_003), NA DF 분리 (cctv_004)]
    자치구명 NA's 10546개 = 행정동명 NA's 10546개로 데이터 프레임 분리 가능
'''



# 5.(1) 지번주소 추출하여 자치구명(26)으로 결측값 대체 ####
...
# 5.(2) 지번주소 추출하여 행정구명(542)으로 결측값 대체 ####
...
# 5.(3) cctv_003과 cctv_004 병합 및 서브 데이터 프레임 cctv_005 생성 ####
